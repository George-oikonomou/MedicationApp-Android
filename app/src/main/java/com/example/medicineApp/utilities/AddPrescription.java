...
public class AddPrescription {
    ...
    public static void show(@NonNull Context ctx, @NonNull RxViewModel vm) {
        ...
        List<TimeTermEntity> initial = vm.timeTerms.getValue();
        ...
        new AlertDialog.Builder(ctx)
                ...
                .setPositiveButton("Save", (d, w) -> {
                    ...
                    // Ensure start <= end (ISO strings compare lexicographically by date)
                    if (startIso[0].compareTo(endIso[0]) > 0) {
                        toast(ctx, "End date MUST be after start date");
                        return;
                    }

                    // Keep ViewModel API as-is to avoid breaking other files
                    vm.addPrescription(
                            title,
    ...
    private static void fillTerms(List<TimeTermEntity> src, List<Integer> ids, List<String> labels, ArrayAdapter<String> ad) {
        ids.clear();
        labels.clear();
        for (TimeTermEntity t : src) {
            ids.add(t.id);
            labels.add(TimeTermStatus.labelForId(t.id));
        }
